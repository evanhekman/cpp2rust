...
def extract_verus_errors(verus_output: str) -> str:
    lines = verus_output.strip().split('\n')
    rendered_messages = []
    
    for line in lines:
        try:
            diagnostic = json.loads(line)
            # Verus-specific diagnostics have null code and level error/note
            if (diagnostic.get('code') is None and 
                diagnostic.get('level') == 'error' and
                diagnostic.get('rendered')):
                rendered_messages.append(diagnostic['rendered'])
        except json.JSONDecodeError:
            continue
